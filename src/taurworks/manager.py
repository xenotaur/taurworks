...
def discover_workspace_projects(workspace):
    ...
    # ⚡ Bolt: Use os.scandir instead of iterdir() for faster directory traversal
    # by avoiding redundant stat calls when checking is_dir()
    with os.scandir(workspace) as entries:
        children = [pathlib.Path(entry.path) for entry in entries if entry.is_dir()]
