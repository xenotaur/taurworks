...
acceptance:
  ...
  - The explicit Conda-creation path is covered by unit tests distinguishing it from the default metadata-only path.
  ...
  - `project/audits/side_effects.md` is updated to mark resolved follow-up items and note any deferred ones with rationale.
...
## Scope
- Make legacy `taurworks refresh`/`taurworks create` metadata-only by default, matching the namespaced `taurworks project refresh`/`create` behavior.
...
5. Update `project/audits/side_effects.md` follow-up recommendations #1 and #2 to reflect resolution; add a note on #3-#7 status (most are already satisfied by current behavior per the audit's own findings).
...
- New/updated tests pass and cover both the default and explicit-opt-in paths.
- `project/audits/side_effects.md` reflects the resolved status of audit follow-ups #1 and #2.
- `lrh validate` passes with 0 errors.
